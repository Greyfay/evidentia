...
export default function SourceViewerDrawer() {
  const { activeEvidenceId, resolveEvidence, closeEvidence } = useCasesData();
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      >
        <div
          className="exhibit-tag px-5 pt-4 pb-5 border-b"
          style={{ background: "var(--ink-2)", borderColor: "var(--hairline)" }}
        >
          <div className="flex items-center justify-between">
            <span className="text-[10px] tracking-[0.18em] uppercase" style={{ color: "var(--amber)" }}>
              Exhibit · Source record
            </span>
            <button
              onClick={closeEvidence}
              aria-label="Close"
              className="text-text-2 hover:text-text-0 transition-colors text-lg leading-none px-1"
            >
              ×
            </button>
          </div>
          {hit && (
            <div className="mt-2 font-mono text-xs" style={{ color: "var(--text-2)" }}>
              evidence_id <span style={{ color: "var(--text-0)" }}>{hit.evidence.evidence_id}</span>
            </div>
          )}
        </div>
          ...
          {!hit ? (
            <div className="mt-8 text-center">
              <p className="text-sm" style={{ color: "var(--text-1)" }}>
                No source record for evidence_id{" "}
              ...
              <p className="mt-2 text-xs" style={{ color: "var(--text-2)" }}>
                This value was not included with a resolvable evidence object in the
                compiled bundle.
              </p>
            </div>
          ) : (
            <div className="flex flex-col gap-5">
              <div>
                ...
                <div className="text-sm" style={{ color: "var(--text-0)" }}>
                  {hit.caseTitle}
                </div>
              </div>
              ...
              <div>
                ...
                >
                  {hit.evidence.raw_value}
                </div>
              </div>

              <dl className="grid grid-cols-[auto,1fr] gap-x-4 gap-y-2.5 text-xs">
                <dt style={{ color: "var(--text-2)" }}>Source path</dt>
                <dd className="font-mono break-all" style={{ color: "var(--text-0)" }}>
                  {hit.evidence.source_path}
                </dd>

                <dt style={{ color: "var(--text-2)" }}>Source type</dt>
                <dd style={{ color: "var(--text-0)" }}>
                  {SOURCE_TYPE_LABEL[hit.evidence.source_type] ?? hit.evidence.source_type}
                </dd>

                <dt style={{ color: "var(--text-2)" }}>Locator</dt>
                <dd className="font-mono" style={{ color: "var(--text-0)" }}>
                  {formatLocator(hit.evidence.locator)}
                </dd>

                <dt style={{ color: "var(--text-2)" }}>File SHA-256</dt>
                <dd className="font-mono break-all" style={{ color: "var(--text-1)" }} title={hit.evidence.file_sha256}>
                  {shortHash(hit.evidence.file_sha256)}
                </dd>
              </dl>
            </div>
          )}
